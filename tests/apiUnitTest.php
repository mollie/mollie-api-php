...
class Mollie_ApiUnitTest extends PHPUnit_Framework_TestCase
{
    ...
    public function testCreatePaymentFailsEmptyHttpBody()
    {
        $this->api->expects($this->once())
            ...
            ->with(MollieApiClient::HTTP_POST, "payments?profileId=pfl_wdy%21aA6Zy", '{"amount":100,"description":"Order #1337 24 Roundhousekicks","redirectUrl":"http:\/\/www.chucknorris.rhk\/return.php"}')
    ...
    public function testCreatePaymentFailsError()
    {
        $this->api->expects($this->once())
            ->method("performHttpCall")
            ->with(MollieApiClient::HTTP_POST, "payments", '{"amount":100,"description":"Order #1337 24 Roundhousekicks","redirectUrl":"http:\/\/www.chucknorris.rhk\/return.php"}')
    ...
    public function testCreateRefundSupportsPartialRefunds()
    {
        $this->api->expects($this->once())
            ->method("performHttpCall")
            ->with(MollieApiClient::HTTP_POST, "payments/tr_OCrlrHqKsr/refunds", '{"amount":60.33}')
    ...
    public function testCreateRefundSupportsDescriptions()
    {
        $this->api->expects($this->once())
            ->method("performHttpCall")
            ->with(MollieApiClient::HTTP_POST, "payments/tr_OCrlrHqKsr/refunds", '{"amount":60.33,"description":"Foo bar"}')
    ...
    public function testGetPaymentWorksCorrectly()
    {
        $this->api->expects($this->once())
            ->method("performHttpCall")
            ->with(MollieApiClient::HTTP_GET, "payments/tr_d0b0E3EA3v?count=1&profileId=pfl_wdy%21aA6Zy")
            ->will($this->returnValue('{ "id":"tr_d0b0E3EA3v", "mode":"test", "createdDatetime":"2013-11-21T09:57:08.0Z", "status":"open", "amount":100, "description":"Order #1225", "method":null, "details":null, "links":{ "paymentUrl":"https://www.mollie.nl/payscreen/pay/d0b0E3EA3v" } }'));
    ...
    public function testGetPaymentsWorksCorrectly()
    {
        $this->api->expects($this->once())
            ->method("performHttpCall")
            ->with(MollieApiClient::HTTP_GET, "payments?offset=0&count=0")
            ->will($this->returnValue('{
  "totalCount":1,
  "offset":0,
  "count":1,
  ...
  "links":{
    ...
    public function testMethodsWorksCorrectly()
    {
        $this->api->expects($this->once())
            ->method("performHttpCall")
            ->with(MollieApiClient::HTTP_GET, "methods?offset=0&count=0&locale=de")
        ...
        $this->assertInstanceOf(BaseCollection::class, $methods);
    ...
    public function testDeleteSubscriptionWorksCorrectly()
    {
        $this->api->expects($this->once())
            ->method("performHttpCall")
            ->with(MollieApiClient::HTTP_DELETE, "customers/cst_3EA3vd0b0E/subscriptions/sub_d0b0E3EA3v")
        ...
        /** @var Subscription $deleted_subscription */
    ...
    public function testUndefinedResourceCallsResourceEndpoint()
    {
        $this->api->expects($this->once())
            ->method("performHttpCall")
            ->with(MollieApiClient::HTTP_GET, "foobars/foobar_ID?f=B")
            ->will($this->returnValue("{}"));
    ...
    public function testUndefinedSubResourceCallsSubresourceEndpointWithParentId()
    {
        $this->api->expects($this->once())
            ->method("performHttpCall")
            ->with(MollieApiClient::HTTP_GET, "foos/foo_PARENT/bars/bar_CHILD?f=B")
            ->will($this->returnValue("{}"));
    ...
    public function testCustomerUpdateWorksCorrectly()
    {
        ...
        $this->api->expects($this->once())
            ->method("performHttpCall")
            ->with(MollieApiClient::HTTP_POST, "customers/cst_8wmqcHMN4U", json_encode($expected_customer_api_call_data))
            ->will($this->returnValue(json_encode($return_value)));

        /** @var Customer $updated_customer */
    ...
    public function testProfileApiKeyGetWorksCorrectly()
    {
        ...
        $this->api->expects($this->once())
            ->method("performHttpCall")
            ->with(MollieApiClient::HTTP_GET, "profiles/$profile_id/apikeys/$api_key_mode")
            ->willReturn(json_encode($return_value));

        $api_key = $this->api->apikeys->withParentId($profile_id)->get($api_key_mode);
    ...
    public function testProfileUpdateWorksCorrectly()
    {
        ...
        $this->api->expects($this->once())
            ->method("performHttpCall")
            ->with(MollieApiClient::HTTP_POST, "profiles/pfl_v9hTwCvYqw", json_encode($expected_profile_api_call_data))
            ->will($this->returnValue(json_encode($return_value)));
    ...
    public function testProfileApiKeyResetWorks()
    {
        ...
        $this->api->expects($this->once())
            ->method("performHttpCall")
            ->with(MollieApiClient::HTTP_POST, "profiles/$profile_id/apikeys/$api_key_mode")
            ->willReturn(json_encode($return_value));

        /** @var ApiKey $updated_api_key */
        $updated_api_key = $this->api->apikeys->withParentId($profile_id)->reset($api_key_mode);
