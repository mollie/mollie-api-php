...
class BalanceTransactionEndpointTest extends BaseEndpointTest
{
    ...
    public function testGetBalanceTransactionsOnBalanceResource()
    {
        $this->mockApiCall(
        ...
        $balance->_links = $this->createNamedLinkObject(
            "balance_transactions",
            "https://api.mollie.com/v2/balances/bal_gVMhHKqSSRYJyPsuoPNFH/transactions",
            "application/hal+json"
        );

        $this->assertTransactions($balance->transactions());

        $this->markTestIncomplete(
            "Not sure yet if Mollie will use this link, and if so, if the _link reference is ok"
        );

        $this->markTestIncomplete("Implement BalanceTransaction attributes explicitly");
    }
